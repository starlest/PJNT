namespace ECERP.Views.Menu
{
    ...
    public partial class SettingsView
    {
        ...
        private void Export()
        {
            ...
            using (
                var conn =
                    new MySqlConnection(string.Format(constring, UtilityMethods.GetDBName(),
                        UtilityMethods.GetIpAddress())))
            {
        ...
        {
            if (MessageBox.Show(
                    $"The date now is: {UtilityMethods.GetCurrentDate():dd-MM-yyyy} \n Confirm decreasing day?",
                    "Confirmation", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.No) return;

            if (!UtilityMethods.GetMasterAdminVerification()) return;
            UtilityMethods.advanceSystemDate(-1);
        }

        private void Increase_Day_Button_Clicked(object sender, RoutedEventArgs e)
        {
            if (MessageBox.Show(
                    $"The date now is: {UtilityMethods.GetCurrentDate():dd-MM-yyyy} \n Confirm increasing day?",
                    "Confirmation", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.No) return;
            UtilityMethods.advanceSystemDate(1);
        }
    }
